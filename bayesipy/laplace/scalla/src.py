...
class ScaLLA:
    ...
    a Laplace approximation, and leverages a secondary model (feature_model) to learn
    ...
    def __init__(
        ...
        self.feature_model = self.feature_model.to(self.device).to(self.dtype)
    ...
    def fit(
        ...
        1. Kernel norm minimization:
           - Train the secondary model (feature_model) by matching its induced kernel
           ...
           - This uses random Jacobian-vector products (JVPs) from the primary model
             and backpropagation into feature_model.
        ...
        ----------
        iterations : int
            Number of mini-batch iterations to train the secondary model (feature_model).
        ...
        weight_decay : float, optional
            L2 regularization coefficient applied to feature_model's parameters.
        lr : float, optional
            Learning rate for feature_model's optimizer (Adam). Defaults to 1e-4 if not provided.
        ...
        for _ in tq:
            ...
            # Compute features from the secondary model (feature_model).
            phi = self.feature_model(x)
            ...
            if weight_decay is not None:
                loss += weight_decay * torch.norm(
                    torch.cat([p.view(-1) for p in self.feature_model.parameters()])
                )
    ...
    def forward(self, x: torch.Tensor):
        ...
        (F_mean, F_var)
            ...
            F_var : torch.Tensor
                ...
                features from feature_model and Cov is the posterior covariance. The shape
