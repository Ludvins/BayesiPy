...
class SNGP(nn.Module):
    ...
    def __init__(
        ...
        self.gp_feature_scale = math.sqrt(2.0 / float(n_random_features))
    ...
    def fit(
        ...
        self._gp_input_normalize_layer = (
            torch.nn.LayerNorm(self.hidden_size, eps=self.layer_norm_eps)
            .to(self.device)
            .to(self.dtype)
        )
    ...
    def gp_layer(self, gp_inputs: Tensor) -> tuple[Tensor, Tensor]:
        """
    ...
    def mean_field_logits(self, logits: Tensor, covmat: Tensor) -> Tensor:
        ...
        logits_scale = torch.sqrt(1.0 + variances)
    ...
    @torch.no_grad()
    def monte_carlo_softmax(
        self, logits: Tensor, var: Tensor, num_samples: int = 50, temp_scale: float = 1.0
    ) -> Tensor:
        """
